...
while True:
    cartao = input("\nDigite aqui o numero do seu cartão:'S'sair>> ").upper()  # trata as letras minusculas
    ...
    for cliente in banco_dados:
        if cliente['cartao'] == cartao and cliente[
            "senha"] == senha1:  # \\valida se o cartão e senha digitados são igais ao banco de dados "cliente"
            saldo = cliente["saldo"]
            titular = cliente["nome"].upper()

            menu_iniciar = menu(titular, saldo,
                                cartao)  # \\os valores de nome e saldo sao baixados para menu de operações
            continue
    print('\nINFORME UM CARTÃO E SENHA VALIDOS PARA CONTINUAR'.center(
        46))  # \\retorna a pergunta ate validar acesso ou "s"para sair
# \\Fim Se...